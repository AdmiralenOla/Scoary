...
class ScoaryGUI(Tkinter.Tk):
	...
	def initialize_statusframe(self):
		...
		frame.pb = ttk.Progressbar(frame,orient='horizontal',mode='determinate',maximum=100)#,length=760)
	...
	def RunAnalysis(self):
		# Build the scoary command line and run
		...
		self.Scoary_parameters["Maxhits"] = self.mhtext.get()
	...
	def PrepareScoaryCMDline(self):
		...
		max_hits = self.Scoary_parameters["Maxhits"] if self.Scoary_parameters["Maxhits"] not in [""] else None
	...
	def initialize_controlboard(self):
		...
		#QuitButton = Tkinter.Button(board.emptyspace,text=u"Quit",font=masterbuttonfont, command=self.stop_thread,padx=15,pady=15)
		...
		HelpButton.grid(column=0,row=0) #pady= (80,10)
	...
	def initialize_miscopts(self):
		...
		self.maxhitsvar = Tkinter.IntVar()
	...
	def citationtext(self):
		...
		"Microbial Pan-GWAS. \n" \
		"https://github.com/AdmiralenOla/Scoary" % scoaryversion #scoary.__version__
		return text
    ...
    def write(self, string, end=None):
        ...
        if string == "\n":
            ...
            if last_char != '\n' and string.startswith('\r'):
				# Then it is a percentage and we dont want to update!
				# Rather, we want the progressbar to update
				...
				# Then it is a percentage and we dont want to update!
				# Rather, we want the progressbar to fill
				number = string.lstrip("\r").rstrip("%")
				number = number.split(".")[0]
				number = int(number)
				self.progressbar["value"] = number
                #self.widget.config(text=string[1:-1])
                
            else:
                # last_line_last_char equals newline
                self.widget.config(text=string[:-1])
