...
def main():
	...
	with open(args.genes, "rU") as genes, open(args.traits, "rU") as traits:
		...
		genedic_and_matrix = Csv_to_dic_Roary(genes, args.delimiter, startcol=args.start_col - 1, allowed_isolates=allowed_isolates) # 0-based indexing
...
def CreateTriangularDistanceMatrix(zeroonesmatrix,strainnames):
	"""
	Converts a raw matrix of 0s and 1s (indicating gene presence and absence, rows correspond to
	genes and columns to allowed strains) to an (upper) triangular matrix of pairwise Hamming distances.
...
def PopulateQuadTreeWithDistances(TDM):
	"""
	Takes a triangular distance matrix, creates a quadtree and populates it with the hamming distances between isolates.
	...
	for i in xrange(Quadmatrix.dim):
		for j in xrange(i,Quadmatrix.dim):
			try:
				...
				print "There was an error trying to populate the Quadtree with pairwise distances. Please report this bug to olbb@fhi.no"
...
def Csv_to_dic_Roary(genefile, delimiter, startcol=0, allowed_isolates=None):
	"""
	Converts a gene presence/absence file into dictionaries that are readable by Roary
	"""
	r = {}
...
def Csv_to_dic(csvfile, delimiter, allowed_isolates):
	...
	for num_trait in xrange(1,len(tab)):
		...
		if allowed_isolates is not None:
			p = {strain: indicator for (strain,indicator) in p.iteritems() if strain in allowed_isolates}
		r[name_trait] = p
...
def Setup_results(genedic, traitsdic):
	"""
	This is the method that actually does all the counting of genes, calculation of p-values and post-test adjustment of p-values. The majority
	of running time is currently spent doing Fisher's exact test. The running time is somewhat shortened by storing the p-values of known 2x2 cell
	distributions, so that identical Fisher's tests will not have to be run.
	"""
	...
	for trait in traitsdic:
		...
		# Additionally, we need a dictionary to, for each gene, hold the gene-trait status (e.g "AB" or "ab" of each strain)
...
def upgma(d):
	"""
	Returns a UPGMA tree from a QuadTree distance matrix d. Heavily based on original implementation by Christian Storm Pedersen.
	"""
	n = d.dim
...
def ConvertUPGMAtoPhyloTree(tree, GTC):
	"""
	A method that converts the upgma tree (in nested list form) to a PhyloTree. It also needs the 
	status (AB, Ab, aB or ab) of all strains that are to be included in the analysis.
	"""
