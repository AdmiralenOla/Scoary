...
# Remember: FDR too lenient when correlated data.
# Bonferroni too strict when uncorrelated
# All adjusted p-values may be too strict because Scoary tests many null hypotheses that a sound researcher would never frame. A gene present in only one strain for example
# will be tested for even though the pattern is obviously not related to the outcome trait. Many such silly hypotheses penalize the p-values of the sound null hypotheses.
...
def Setup_results(genedic, traitsdic):
	...
	for trait in traitsdic:
		...
		for gene in genedic:
			...
			# IDEA: If no strains are positive for the gene, just skip the gene? Adjusted p-values should not be penalized for tests that are made without any reasonable assumption
			# This only happens when using the --restrict_to option, as the pan-genome is calculated from strains that are now not included.
			# However, is the opposite the case if the number of gene negative isolates is 0. (i.e. all isolates possess the gene)?
			# No one would rightly form a hypothesis that the presence/absence of such a core gene determined a trait that was differentially distributed in the population.
			# Therefore, yes. The opposite applies for all-positive genes as well.
...
def StoreResults(Results, max_hits, p_cutoff, correctionmethod):
	for Trait in Results: # For every trait. Order does not matter
